    ...
    info_section = ft.Container(
        content=ft.Column(
            [
                ft.Row(
                    [
                        ft.Icon(ft.Icons.BACK_HAND, color=ACCENT, size=16),
                        ft.Text(
                            "Hand Info",
                            size=13,
                            weight=ft.FontWeight.W_600,
...
async def main(page: ft.Page):
    ...
    _ui_task = asyncio.create_task(_ui_update_loop())
    ...
    async def on_window_event(e):
        if getattr(e, "data", "") == "close":
            ...
            # En Flet 0.85.3 con prevent_close=True, es obligatorio desactivar
            # prevent_close y hacer update_async() ANTES de llamar destroy().
            # De lo contrario destroy() puede ser interceptado por el mismo gate
            # que previene el cierre del OS y la ventana no se destruye.
            try:
    ...
    # En Flet 0.85.3: page.window.on_event es la API correcta (no page.on_window_event).
    # El setter registra el handler en el sistema de eventos interno de Window.
    # prevent_close=True se envía al cliente con el siguiente update_async().
